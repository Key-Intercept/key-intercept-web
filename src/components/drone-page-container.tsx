...
export default function DronePageContainer({
    ...
    const [droneConfig, setDroneConfig] = useState<DroneConfigState>(initialDroneConfig);
    const [isSaving, setIsSaving] = useState(false);

    const hasChanges = () => {
        return fieldLabels.some(({ key }) => droneConfig[key] !== initialDroneConfig[key]);
    };

    const healthLabel = useMemo(() => `${droneConfig.drone_health} / 100`, [droneConfig.drone_health]);
    ...
    async function adjustHealth(direction: "damage" | "heal") {
        ...
        const nextHealth = direction === "damage"
                        ...
                        {fieldLabels.map(({ key, label }) => (
                            <div key={key} style={fieldStyle}>
                                ...
                                <input
                                    ...
                                    onChange={(event) =>
                                        setDroneConfig((current) => ({ ...current, [key]: event.target.value }))
                                    }
                                />
                            </div>
                        ))}
