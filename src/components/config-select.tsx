...
export default async function ConfigSelect({
  ...
  if (configsError) {
    console.error(
      "[supabase] Error fetching dom config accesses:",
      configsError,
    );
    console.error("Discord ID:", discordID);
    return errorOccurred("--- Error loading configs ---");
  ...
  const options = await Promise.all(
    uniqueConfigs.map(async (config) => {
      ...
      if (subDetailsIDError || !subDetailsID) {
        console.error(
          "[supabase] Error fetching sub details ID:",
          subDetailsIDError,
        );
        console.error("Discord ID:", discordID);
        return null;
      ...
      if (subDetailsError || !subDetails) {
        console.error(
          "[supabase] Error fetching sub details:",
          subDetailsError,
        );
        console.error("Discord ID:", discordID);
        return null;
  ...
  const renderedOptions = [
    ...
    ...options.filter((option): option is JSX.Element => option !== null),
  ];
  ...
  return (
    <select id="config-select" style={selectStyle}>
      {renderedOptions}
    </select>
  );
}
